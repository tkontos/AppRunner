...
#define REG_ITEM_EX(name_, variable, DEFAULT_VALUE) \
         if(bOnlyInit) variable = DEFAULT_VALUE; else {\
         dwResult = SaveLoadItem(hKey, _T(name_), variable, bSave); \
         if(dwResult != ERROR_SUCCESS) \
         if(bSave) return dwResult; else variable = DEFAULT_VALUE; }
...
#define REG_ITEM_VECTOR(name_, variable) \
         if(!bOnlyInit){\
         dwResult = SaveLoadItemSTL(hKey, _T(name_), variable, bSave); \
         if(dwResult != ERROR_SUCCESS) \
         if(bSave) return dwResult;}

#define REG_ITEM_LIST(name_, variable) \
         if(!bOnlyInit){\
         dwResult = SaveLoadItemSTL(hKey, _T(name_), variable, bSave); \
         if(dwResult != ERROR_SUCCESS) \
         if(bSave) return dwResult;}
...
{
protected:
   void Init(HKEY hKey, LPCTSTR szKey, ...)
   {
      m_hKey = hKey;
      if(szKey != NULL) 
      {
   ...
   // double
   inline DWORD SaveLoadItem(HKEY hKey, LPCTSTR szName, double &Variable, bool bSave)
   {
      const DWORD N = 256;
      TCHAR szTmp[N+1];
      DWORD dwType = REG_SZ;
      if(bSave)
      {
         DWORD dwSize = _stprintf_s(szTmp, N, _T("%f"), Variable);
         return ::RegSetValueEx(hKey, szName, NULL, dwType, (LPBYTE)szTmp, dwSize);
      }
      else
      {
         DWORD dwSize = N;
         DWORD dwResult = ::RegQueryValueEx(hKey, szName, NULL, &dwType, (LPBYTE)szTmp, &dwSize);
         if (dwResult == ERROR_SUCCESS)
         {
            TCHAR *p;
            Variable = _tcstod(szTmp, &p);
         }
         return dwResult;
      }
   }
   ...
   DWORD SaveLoadItemSTL(HKEY hKey, LPCTSTR szName, TSTLArray& Variable, bool bSave)
   {
      ...
      if(bSave)
      {
         ...
         for(TSTLArray::iterator it = Variable.begin(); it != Variable.end(); it++)
         {
            ...
            dwResult = SaveLoadItem(hKeySub, sItemKey, *it, bSave);
      ...
      else
      {
         for(DWORD i = 0; i < dwItemCount; i++)
         {
            ...
            dwResult = SaveLoadItem(hKeySub, sItemKey, item, bSave);
            //dwResult = item.InternalSaveLoad(hKeyItem, bSave, false);
            if(dwResult == ERROR_SUCCESS)
